...
import facade.api.ModelCrudFacadeRemoteA;
import facade.api.ModelCrudFacadeRemoteC;
...
public class ModelCrudFacadeImplSystemT {
    ...
    ModelCrudFacadeRemote remoteFacade;
    ModelCrudFacadeRemoteA remoteFacadeA;
    ModelCrudFacadeRemoteB remoteFacadeB;
    ...
    public void before() {
        ...
        //java:global/generic-logging-interceptor-bug/ModelCrudFacadeImpl!facade.ModelCrudFacadeRemote
        remoteFacade = JndiUtil.SINGLETON.resolveBean(WAR_NAME, "ModelCrudFacadeImpl",
                ModelCrudFacadeRemote.class);
        remoteFacadeA = JndiUtil.SINGLETON.resolveBean(WAR_NAME, "ModelCrudFacadeAImpl",
                ModelCrudFacadeRemoteA.class);
        remoteFacadeB = JndiUtil.SINGLETON.resolveBean(WAR_NAME, "ModelCrudFacadeBImpl",
                ModelCrudFacadeRemoteB.class);
        remoteFacadeC = JndiUtil.SINGLETON.resolveBean(WAR_NAME, "ModelCrudFacadeCImpl",
                ModelCrudFacadeRemoteC.class);
    ...
    public void tryToGetInterceptorToDie() throws InterruptedException {
        ...
        try {
            for (int i = 0; i < NUMBER_OF_FACADE_CALLS_TO_DO; i++) {
                executorService.execute(new Runnable() {
                    @Override
                    public void run() {
                        Long result = remoteFacade.countEntities();
                        System.out.println("numberOfEntitiesReturned: " + result);
                    }
                });

                executorService.execute(new Runnable() {                    @Override
                    public void run() {
                        Long result = remoteFacadeA.countEntities();
                        System.out.println("numberOfEntitiesReturned: " + result);
                    }
                });
                 ...
                 executorService.execute(new Runnable() {                    @Override
                    public void run() {
                        Long result = remoteFacadeB.countEntities();
                        System.out.println("numberOfEntitiesReturned: " + result);
                    }
                });
                ...
                executorService.execute(new Runnable() {
                    @Override
                    public void run() {
                        Long result = remoteFacadeC.countEntities();
                        System.out.println("numberOfEntitiesReturned: " + result);
                    }
                });
            }            
        ...
        // 2 - wait for all the threads to be finished publishing the POR1 to wm6.
